...
glm::vec2 Util::clamp(glm::vec2 vec, float max_length)
{
	...
	if(sqr_magnitude > max_length * max_length)
	{
		...
		return glm::vec2(normalized_x * max_length, normalized_y * normalized_y);
	}
	return vec;
}
...
glm::vec2 Util::lerp(const glm::vec2 p0, const glm::vec2 p1, float t)
{
...
{
	const auto randomX = RandomRange(p0.x, p1.x);
	const auto randomY = RandomRange(p0.y, p1.y);
